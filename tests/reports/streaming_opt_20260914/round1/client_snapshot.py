...
class GovernedClient:
    def __init__(self, location, token, timeout=120):
    ...
    def prefetch(self, info, *, max_batches=2, max_bytes=16 * 1024 * 1024):
...
class PrefetchedBatches:
    ...
    """
    def __init__(self, reader, *, max_batches=2, max_bytes=16 * 1024 * 1024):
        if max_batches < 1 or max_bytes < 1:
            raise ValueError("Prefetch limits must be positive")
        self.reader = reader
    ...
    def _produce(self):
        try:
            iterator = iter(iter_batches(self.reader))
            while True:
