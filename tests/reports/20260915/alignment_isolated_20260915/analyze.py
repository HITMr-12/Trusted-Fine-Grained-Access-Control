...
for run in range(1,4):
 ...
 done=json.loads((e/'done.json').read_text());assert done['ok'] and done['formal_records']==384,done
 assert json.loads((e/'ranger-policies-before.json').read_text())==json.loads((e/'ranger-policies-after.json').read_text())
 rows=[json.loads(x) for x in (e/'records.jsonl').read_text().splitlines()];assert len(rows)==704
 ...
 for mode in ['COLLECT','STREAM']:
  ...
  assert len(xs)==len(audits)==176,(run,mode,len(xs),len(audits))
 ...
 for case in ['bob_001','bob_01']:
  ...
  for a,b in comparisons:
   ...
   summary.append({'run':run,'case':case,'comparison':a+'_vs_'+b,'paired_mean_pct':statistics.mean(xs),'ci_block4':ci(xs),'block16_means':[statistics.mean(xs[i:i+16]) for i in range(0,48,16)]})
metrics=[]
...
print(json.dumps([x for x in summary if x['comparison'] in ['STREAM_vs_COLLECT','STREAM_vs_NATIVE','COLLECT_vs_NATIVE']],indent=2))
