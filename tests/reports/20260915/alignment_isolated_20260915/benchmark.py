...
r=Path('/home/lyb/fgac-lab');d=r/'deploy/fgac-alignment-isolated';out=Path(sys.argv[1]);smoke='--smoke' in sys.argv;out.mkdir(parents=True,exist_ok=True);(out/'events').mkdir(exist_ok=True)
...
def launch(kind,user):
    ...
    env=os.environ.copy();env.update({'SPARK_CONF_DIR':str(r/'deploy/fgac-current'/conf),'JAVA_HOME':'/usr/lib/jvm/java-11-openjdk-arm64','PYSPARK_PYTHON':'/home/lyb/fgac/venv/bin/python','PYTHONPATH':str(r/'envs/fgac-python')+':'+str(d),'HADOOP_USER_NAME':user,'SPARK_LOCAL_DIRS':str(out/(kind+'-tmp')),'TMPDIR':str(out/(kind+'-tmp'))})
    (out/(kind+'-tmp')).mkdir(exist_ok=True)
...
try:
    ...
    cases=[c for c in json.loads((d/'cases.json').read_text()) if c['name'] in ('bob_001','bob_01')]
    ...
    warm_rng=random.Random(91700+int(out.name.split('-')[-1]))
    for i in range(80):
    ...
    rng=random.Random(91600+int(out.name.split('-')[-1]));orders={}
    for c in cases:
