...
p=Path(__file__).resolve().parent;rows=json.loads((p/'verified-records.json').read_text());
summary=[]
for run in sorted({x['run'] for x in rows}):
 for role,folder,groups in [('E',p/'e'/('round-'+str(run)),{x['id']:x for x in rows if x['run']==run})]+[('R',p/'r'/('round-'+str(run))/mode,{x['remote_job_id']:x for x in rows if x['run']==run and x['mode']==mode}) for mode in ['COLLECT','STREAM']]:
    ...
    for group,m in metric.items():
        record=groups[group];summary.append(dict(run=run,role=role,id=record['id'],case=record['case'],mode=record['mode'],warmup=record['warmup'],**m))
out=[]
for run in sorted({x['run'] for x in rows}):
 for role,mode in [('E','NATIVE'),('E','INLINE'),('E','COLLECT'),('E','STREAM'),('R','COLLECT'),('R','STREAM')]:
  for case in sorted({x['case'] for x in rows}):
   xs=[x for x in summary if x['run']==run and x['role']==role and x['mode']==mode and x['case']==case and not x['warmup']]
   assert len(xs)==48,(run,role,mode,case,len(xs))
   out.append(dict(run=run,role=role,mode=mode,case=case,**{k:statistics.median(x[k] for x in xs) for k in ['tasks','executor_run_ms','executor_cpu_ms','gc_ms','input_bytes','input_records','memory_spill','disk_spill']}))
(p/'stage-summary.json').write_text(json.dumps(out,indent=2));(p/'stage-records.json').write_text(json.dumps(summary));(p/'stage-validation.json').write_text(json.dumps({'ok':True,'query_groups':len(summary),'failed_tasks_or_jobs':0,'memory_spill_bytes':sum(x['memory_spill'] for x in summary),'disk_spill_bytes':sum(x['disk_spill'] for x in summary)},indent=2));print('Validated query groups:',len(summary))
