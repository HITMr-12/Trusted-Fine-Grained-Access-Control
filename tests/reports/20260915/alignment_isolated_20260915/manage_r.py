...
r=Path('/data1/lyb/fgac-lab');d=r/'deploy/fgac-alignment-isolated';round_id=int(sys.argv[2]);out=r/'runs/alignment-isolated-0915'/('round-'+str(round_id))
def live(pid):
 try:return Path('/proc',str(pid),'stat').read_text().rsplit(')',1)[1].split()[0]!='Z'
 except FileNotFoundError:return False
if sys.argv[1]=='start':
 out.mkdir(parents=True,exist_ok=False)
 for mode,port in [('COLLECT',18840),('STREAM',18841)]:
 ...
 for mode in ['COLLECT','STREAM']:
  ...
  for pid in [j,py]:assert '/deploy/fgac-alignment-isolated/' in Path('/proc',str(pid),'cmdline').read_bytes().decode().replace('\0',' ')
  ...
  for pid in remaining:
   assert '/deploy/fgac-alignment-isolated/' in Path('/proc',str(pid),'cmdline').read_bytes().decode().replace('\0',' ')
