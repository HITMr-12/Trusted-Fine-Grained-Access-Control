...
try:
    ...
    cases=[c for c in cases if c['name'] in ['bob_90','bob_100']]
    save('cases.json',cases)
    save('protocol.json',{'modes':['NATIVE','FGAC','INLINE'],'snapshot':6896028106384817739,'formal_repetitions':60,'warmup_repetitions':20,'minimum_warmup_queries_per_session':40,'seed':91502,'compute_budget':'User-selected deployment comparison: identical local[2] E Spark settings; FGAC adds R local[2]; no CPU affinity or aggregate quota','sink':'four-column double xxhash64 digest included in timer','cache':'Iceberg catalog cache enabled 30s, no data/result cache','timing':'start before SQL/Flight describe, end after full digest and bridge close','smoke':smoke})
    for kind,user in [('FGAC','lyb'),('INLINE','lyb'),('NATIVE','bob')]:launch(kind,user)
    if not smoke:
    ...
    rng=random.Random(91502);orders={}
    for c in cases:
        order=list(itertools.permutations(['NATIVE','FGAC','INLINE']))*10;rng.shuffle(order);orders[c['name']]=order
    ...
    for rep in (range(1) if smoke else range(-20,60)):
        ...
        if False:  # Each of the three sessions already has 40 warmup queries.
            extra=[]
