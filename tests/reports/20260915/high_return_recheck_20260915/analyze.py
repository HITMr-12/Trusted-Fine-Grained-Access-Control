...
assert done['ok'] and len(rows)==480 and len(cases)==2
offset=json.loads((p/'r/analysis-offsets.json').read_text())
...
fgac=[x for x in rows if x['mode']=='FGAC'];assert len(fgac)==len(audits)==160
...
assert len(prep)==len(reval)==len(streams)==160
...
def overall(selected):
    ...
    byrep=[statistics.mean(x['FGAC_over_NATIVE_minus1'] for x in a if x['rep']==rep) for rep in range(60)]
    block_boot=[]
