...
summary=read('summary.json');comparison=read('comparison.json');stages=read('stage-summary.json');overall=read('overall.json')
pc=lambda x:f'{100*x:+.2f}%'
table='\n'.join(f"| {x['case']} | {x['NATIVE_total_ms']:.1f} | {x['FGAC_total_ms']:.1f} | {x['INLINE_total_ms']:.1f} | {pc(x['FGAC_over_NATIVE_mean'])} | [{pc(y['current_block_ci95'][0])}, {pc(y['current_block_ci95'][1])}] |" for x,y in zip(summary,comparison))
oldtable='\n'.join(f"| {x['case']} | {pc(x['previous_degradation'])} | {pc(x['current_degradation'])} | {pc(x['latency_changes']['NATIVE']['relative_change'])} | {pc(x['latency_changes']['FGAC']['relative_change'])} |" for x in comparison)
blocks='\n'.join(f"| {x['case']} | "+' | '.join(pc(b['paired_mean_degradation']) for b in x['blocks'])+' |' for x in comparison)
probes='\n'.join(f"| {x['case']} | {x['role']}/{x['mode']} | {x['input_records']:,.0f} | {x['input_bytes']:,.0f} | {x['executor_cpu_ms']:.1f} | {x['gc_ms']:.1f} |" for x in stages)
phase='\n'.join(f"| {x['case']} | {x['NATIVE_prepare_ms']:.1f} / {x['NATIVE_action_ms']:.1f} | {x['FGAC_prepare_ms']:.1f} / {x['FGAC_action_ms']:.1f} | {x['FGAC_arrow_bytes']:,.0f} |" for x in summary)
text=f'''# 高返回比例场景独立复测
...
**高返回比例下 FGAC 慢于原生的结论得到复现；上轮的 14.97% / 19.18% 不应理解为固定常数。** 本轮分别为 {pc(summary[0]['FGAC_over_NATIVE_mean'])} / {pc(summary[1]['FGAC_over_NATIVE_mean'])}，上轮两个点估计均在本轮区块 bootstrap 95% 区间内。两场景区间下界均超过 5%，本轮也没有达到 5% 目标。这不是两轮性能等效的统计证明。
...
'''
(p/'RECHECK_REPORT.md').write_text(text,encoding='utf-8')
