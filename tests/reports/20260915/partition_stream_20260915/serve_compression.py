...
r=Path('/data1/lyb/fgac-lab');out=r/'runs/partition-stream-0915';lock=threading.Lock();hz=os.sysconf('SC_CLK_TCK');jpid=None
def usage():
...
profiles=[('COLLECT',18830),('STREAM',18831)]
servers=[]
...
(r/'manifests/fgac-partition-stream.pid').write_text(str(os.getpid()))
def stop(*_):
