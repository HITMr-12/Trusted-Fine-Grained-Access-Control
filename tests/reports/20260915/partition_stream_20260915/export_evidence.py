...
role=sys.argv[1];r=Path('/data1/lyb/fgac-lab' if role=='R' else '/home/lyb/fgac-lab');out=r/'runs/partition-stream-0915';d=r/'deploy/fgac-partition-stream'
secrets=[]
...
with tarfile.open(out/('evidence-'+role+'.tar.gz'),'w:gz') as tar:
    for p in out.rglob('*'):
        ...
        if p.name=='resources-'+role+'.jsonl' or p.parent==out or 'formal' in p.relative_to(out).parts or 'initial-formal' in p.relative_to(out).parts or 'smoke3' in p.relative_to(out).parts:
