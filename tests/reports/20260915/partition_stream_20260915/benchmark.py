...
r=Path('/home/lyb/fgac-lab');d=r/'deploy/fgac-partition-stream';out=Path(sys.argv[1]);smoke='--smoke' in sys.argv;out.mkdir(parents=True,exist_ok=True)
...
def launch(kind,user):
    ...
    p=subprocess.Popen(['/home/lyb/fgac/spark/spark-3.5.8-bin-hadoop3/bin/spark-submit','--conf','spark.eventLog.dir=file:/home/lyb/fgac-lab/runs/partition-stream-0915/events',str(d/'worker.py'),kind,user,str(out)],env=env,cwd=d,stdin=subprocess.PIPE,stdout=subprocess.PIPE,stderr=(out/(kind+'-'+user+'.log')).open('w'),text=True,bufsize=1)
...
try:
    ...
    modes=['COLLECT','STREAM']
    save('cases.json',cases)
    save('protocol.json',{'modes':modes,'formal_repetitions':60,'warmup_repetitions':20,'seed':91507,'same_R_Spark':True,'same_E_Spark':True,'resource_policy':'no affinity or CPU quota','prefetch':'2 batches / 16 MiB unchanged; actual byte occupancy varies with batch size','purpose':'partition-internal stream vs current collect; both 8192 rows, no compression; not new Ranger latency comparison'})
    launch('FGAC','lyb')
    w=workers[('FGAC','lyb')]
    ...
    rng=random.Random(91507);orders={}
    for c in cases:
        seq=[['COLLECT','STREAM'],['STREAM','COLLECT']]*30; rng.shuffle(seq)
        orders[c['name']]=seq
    save('orders.json',orders)
    for rep in (range(1) if smoke else range(-20,60)):
