...
s=read('summary.json');w=read('wire-summary.json');validation=read('validation.json');st=read('stage-validation.json')
pc=lambda x:f'{100*x:+.2f}%'
interval=lambda x:'['+', '.join(pc(v) for v in x)+']'
latency='\n'.join(f"| {x['case']} | {x['mode']} | {x['total_ms']:.1f} | {x['p95_ms']:.1f} | {pc(x['over_NONE_mean'])} {interval(x['over_NONE_block_ci95'])} | {pc(x['over_NATIVE_mean'])} |" for x in s)
wire='\n'.join(f"| {x['case']} | {x['mode']} | {x['tcp_received_bytes_median']/1e6:.2f} | {100*x['saved_fraction_vs_NONE']:.2f}% |" for x in w)
cpu='\n'.join(f"| {x['case']} | {x['mode']} | {x.get('first_arrow_ms',0):.1f} | {x['prepare_ms']:.1f} | {x['action_ms']:.1f} | {x['total_process_cpu_ms']:.1f} |" for x in s if x['mode']!='NATIVE')
blocks='\n'.join(f"| {x['case']} | {x['mode']} | "+' | '.join(pc(v) for v in x['over_NONE_blocks'])+' |' for x in s if x['mode'] in ['LZ4','ZSTD'])
avg={m:statistics.mean(x['over_NONE_mean'] for x in s if x['mode']==m) for m in ['LZ4','ZSTD']}
text=f'''# Arrow Flight 压缩实现与性能测试
...
两场景等权的配对耗时变化，相对同轮无压缩 FGAC：LZ4 **{pc(avg['LZ4'])}**，Zstd level 1 **{pc(avg['ZSTD'])}**。负值表示更快。具体置信区间与漂移见下表，不能把两场景均值推广到任意负载。
...
{st['query_groups']} 个 E/R 常规预热及正式查询组通过 Spark 事件校验，失败任务/作业数为 {st['failed_tasks_or_jobs']}，内存 spill {st['memory_spill_bytes']:.0f} 字节、磁盘 spill {st['disk_spill_bytes']:.0f} 字节。额外原生预热单独保存并校验结果，不混入正式统计。详细源读取、CPU、GC 见 stage-summary.json，队列读取/背压见 verified-records.json。
...
'''
(p/'COMPRESSION_REPORT.md').write_text(text,encoding='utf-8')
print('Report written',avg)
