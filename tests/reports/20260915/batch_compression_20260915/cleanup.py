...
role=sys.argv[1];r=Path('/data1/lyb/fgac-lab' if role=='R' else '/home/lyb/fgac-lab');out=r/'runs/batch-compression-0915';result={'role':role}
(out/'stop-sampler').touch()
if role=='R':
 ready=json.loads((out/'remote-ready.json').read_text());j=ready['java'];py=ready['python']
 assert '/deploy/fgac-batch-compression/' in Path('/proc',str(j),'cmdline').read_bytes().decode().replace('\0',' ')
 assert '/deploy/fgac-batch-compression/' in Path('/proc',str(py),'cmdline').read_bytes().decode().replace('\0',' ')
 ...
 for pid in remaining:
  assert '/deploy/fgac-batch-compression/' in Path('/proc',str(pid),'cmdline').read_bytes().decode().replace('\0',' ')
