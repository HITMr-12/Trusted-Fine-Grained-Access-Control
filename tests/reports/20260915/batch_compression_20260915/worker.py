...
try:
    for line in requests():
        ...
        try:
            ...
            if mode != 'NATIVE':
                os.environ['FGAC_FLIGHT_URL']='grpc://172.168.22.23:'+str(18820+[8192,32768,65536].index(int(mode.split('_')[1]))*3+['NONE','LZ4','ZSTD'].index(mode.split('_')[0]))
            ...
            row={'ok':True,'id':ident,'case':case['name'],'mode':mode,'principal':case['principal'],'rep':cmd['rep'],'warmup':cmd['rep']<0,'start':start_wall,'end':time.time(),'total_ms':(end-t)*1000,'prepare_ms':(ready-t)*1000,'digest_setup_ms':(action-ready)*1000,'action_ms':(end-action)*1000,'e_cpu_ms':cpu_ms,'digest':value,'schema':schema,'sql':sql if mode=='NATIVE' else None}
            if bridge:
                row.update(stream_rows=bridge.rows,stream_batches=bridge.batches,arrow_bytes=bridge.nbytes,first_arrow_ms=(bridge.first_batch_at-t)*1000 if bridge.first_batch_at else None,bridge_next_wait_ms=bridge.next_wait_ms,bridge_ipc_write_ms=bridge.ipc_write_ms,prefetch_peak_bytes=bridge.prefetched.peak_queued_bytes,prefetch_peak_batches=bridge.prefetched.peak_queued_batches,prefetch_read_ms=bridge.prefetched.read_ms,prefetch_backpressure_ms=bridge.prefetched.backpressure_ms)
