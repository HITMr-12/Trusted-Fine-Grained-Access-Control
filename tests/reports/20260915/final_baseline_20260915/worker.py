...
try:
    for line in requests():
        ...
        try:
            ...
            if mode=='FGAC':
                os.environ['FGAC_FLIGHT_URL']='grpc://172.168.22.23:18832'
            ...
            row={'ok':True,'id':ident,'case':case['name'],'mode':mode,'principal':case['principal'],'rep':cmd['rep'],'warmup':cmd['rep']<0,'start':start_wall,'end':time.time(),'total_ms':(end-t)*1000,'prepare_ms':(ready-t)*1000,'digest_setup_ms':(action-ready)*1000,'action_ms':(end-action)*1000,'e_cpu_ms':cpu_ms,'digest':value,'schema':schema,'sql':sql if mode!='FGAC' else None}
            if bridge:
