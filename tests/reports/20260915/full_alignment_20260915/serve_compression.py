...
r=Path('/data1/lyb/fgac-lab');out=Path(sys.argv[1]);lock=threading.Lock();hz=os.sysconf('SC_CLK_TCK');jpid=None
def usage():
...
profiles=[(sys.argv[2],int(sys.argv[3]))]
servers=[]
...
# Isolated runtime state lives only in the run directory.
def stop(*_):
