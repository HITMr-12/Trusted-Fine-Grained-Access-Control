...
r=Path('/home/lyb/fgac-lab');d=r/'deploy/fgac-full-alignment';out=Path(sys.argv[1]);smoke='--smoke' in sys.argv;out.mkdir(parents=True,exist_ok=False);(out/'events').mkdir(exist_ok=True)
...
def launch(kind,user):
    conf={'NATIVE':'native-conf','INLINE':'inline-conf','COLLECT':'conf','STREAM':'conf'}[kind]
    env=os.environ.copy();env.update({'SPARK_CONF_DIR':str(r/'deploy/fgac-current'/conf),'JAVA_HOME':'/usr/lib/jvm/java-11-openjdk-arm64','PYSPARK_PYTHON':'/home/lyb/fgac/venv/bin/python','PYTHONPATH':str(r/'envs/fgac-python')+':'+str(d),'HADOOP_USER_NAME':user,'SPARK_LOCAL_DIRS':str(out/(kind+'-'+user+'-tmp')),'TMPDIR':str(out/(kind+'-'+user+'-tmp'))})
    (out/(kind+'-'+user+'-tmp')).mkdir(exist_ok=True)
    p=subprocess.Popen(['/home/lyb/fgac/spark/spark-3.5.8-bin-hadoop3/bin/spark-submit','--conf','spark.eventLog.dir=file:'+str(out/'events'),str(d/'worker.py'),kind,user,str(out)],env=env,cwd=d,stdin=subprocess.PIPE,stdout=subprocess.PIPE,stderr=(out/(kind+'-'+user+'.log')).open('w'),text=True,bufsize=1)
...
try:
    ...
    save('protocol.json',{'modes':['NATIVE','COLLECT','STREAM','INLINE'],'snapshot':6896028106384817739,'formal_repetitions':48,'warmup_schedule':'80 randomized round-robin cycles across six independent E sessions','minimum_warmup_queries_per_session':80,'seed':91900+int(out.name.split('-')[-1]),'compute_budget':'User-selected deployment comparison: identical local[2] E Spark settings; FGAC adds R local[2]; no CPU affinity or aggregate quota','sink':'four-column double xxhash64 digest included in timer','cache':'Iceberg catalog cache enabled 30s, no data/result cache','timing':'start before SQL/Flight describe, end after full digest and bridge close','smoke':smoke,'minimum_between_restart_seconds':300,'cases_per_round':8})
    actors=[('COLLECT','lyb'),('STREAM','lyb'),('INLINE','lyb'),('NATIVE','bob'),('NATIVE','alice'),('NATIVE','bench_full')];shift=(int(out.name.split('-')[-1])-1)%6
    for kind,user in actors[shift:]+actors[:shift]:launch(kind,user)
    if not smoke:
        ...
        result=call(workers[('STREAM','lyb')],{'id':'negative-fgac','case':bad,'mode':'STREAM','rep':-99})
    ...
    warm_rng=random.Random(92000+int(out.name.split('-')[-1]))
    for i in range(80):
    ...
    rng=random.Random(91900+int(out.name.split('-')[-1]));orders={}
    for c in cases:
        order=list(itertools.permutations(['NATIVE','COLLECT','STREAM','INLINE']))*2;rng.shuffle(order);orders[c['name']]=order
    save('orders.json',orders)
    for rep in (range(1) if smoke else range(48)):
        cycle=cases[:];rng.shuffle(cycle)
        for c in cycle:
            pair=[];order=orders[c['name']][rep] if rep>=0 else list(itertools.permutations(['NATIVE','COLLECT','STREAM','INLINE']))[rep%6]
            for mode in order:
