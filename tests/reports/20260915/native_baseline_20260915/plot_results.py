...
import numpy as np
s=json.loads((p/'summary.json').read_text());x=np.arange(len(s));labels=['Bob 0.1%','Bob 1%','Bob 10%','Bob 50%','Bob 90%','Bob 100%','Alice original','Whole source 100%']
...
axes[0].set_ylabel('Median end-to-end latency (ms)');axes[0].legend(frameon=False,fontsize=9);axes[0].set_xticks(x,labels,rotation=15,ha='right');axes[0].grid(axis='y',alpha=.2);axes[0].set_axisbelow(True)
mean=np.array([a['FGAC_over_NATIVE_mean']*100 for a in s]);lo=np.array([a['FGAC_over_NATIVE_ci95'][0]*100 for a in s]);hi=np.array([a['FGAC_over_NATIVE_ci95'][1]*100 for a in s])
...
fig.suptitle('Polaris-backed deployment comparison — 30 paired repetitions per case',fontsize=14)
