...
r=Path('/data1/lyb/fgac-lab');out=r/'runs/native-baseline-0915';lock=threading.Lock();hz=os.sysconf('SC_CLK_TCK');jpid=None
def usage():
...
server=GovernedFlightServer('grpc://172.168.22.23:18815',backend,audit=out/'remote-audit.jsonl')
pid={'python':os.getpid(),'java':int(backend.spark._jvm.java.lang.management.ManagementFactory.getRuntimeMXBean().getName().split('@')[0])}
jpid=pid['java']
(out/'remote-ready.json').write_text(json.dumps(dict(**pid,spark=backend.spark.version,port=18815)))
(r/'manifests/fgac-current.pid').write_text(str(os.getpid()))
