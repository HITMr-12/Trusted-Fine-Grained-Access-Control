...
import secrets,socket,threading,time,os
import pyarrow as pa
from streaming.client import GovernedClient,iter_batches

class FlightBridge:
 def __init__(self,plan,token,prefetch=False):
  self.client=GovernedClient(os.getenv('FGAC_FLIGHT_URL','grpc://172.168.22.23:18815'),token)
  self.info=self.client.describe(plan)
  assert len(self.info.endpoints)==1
  self.prefetched=self.client.prefetch(self.info) if prefetch else None
 ...
 def pump(self):
  ...
  try:
   ...
   iterator=iter(self.prefetched) if self.prefetched is not None else iter(iter_batches(self.reader))
   while True:
    q=time.perf_counter()
 ...
 def dataframe(self,spark):
  ...
  expected=pa.schema([('VendorID',pa.int32()),('trip_distance',pa.float64()),('fare_amount',pa.float64()),('payment_type',pa.int64())])
  assert [(f.name,f.type) for f in self.info.schema]==[(f.name,f.type) for f in expected]
  schema=StructType([StructField('VendorID',IntegerType()),StructField('trip_distance',DoubleType()),StructField('fare_amount',DoubleType()),StructField('payment_type',LongType())])
  return spark.read.format('fgac.bench.ArrowSocketSource').option('schema',schema.json()).option('ports',','.join(str(x.getsockname()[1]) for x in self.listeners)).option('secret',self.secret).load()
 def finish(self):
