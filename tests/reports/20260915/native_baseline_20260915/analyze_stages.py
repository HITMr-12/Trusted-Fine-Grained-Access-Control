...
p=Path(__file__).resolve().parent;rows=json.loads((p/'verified-records.json').read_text());wanted={x['id']:x for x in rows};wanted_r={x['remote_job_id']:x for x in rows if x['mode']=='FGAC'}
summary=[]
...
out=[]
for role,mode in [('E','NATIVE'),('E','INLINE'),('E','FGAC'),('R','FGAC')]:
...
(p/'stage-summary.json').write_text(json.dumps(out,indent=2));(p/'stage-validation.json').write_text(json.dumps({'ok':True,'query_groups':len(summary),'failed_tasks_or_jobs':0},indent=2));print(json.dumps(out,indent=2))
