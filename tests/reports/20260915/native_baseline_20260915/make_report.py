...
s=read('summary.json');o=read('overall.json');st=read('stage-summary.json');res=read('resource-summary.json')
initial=read('initial-overall.json');rows=read('verified-records.json')
percent=lambda x:f'{100*x:+.2f}%'
names=['Bob 0.1%','Bob 1%','Bob 10%','Bob 50%','Bob 90%','Bob 100%（授权集合）','Alice 原场景','原表 100%']
table='\n'.join('| '+names[i]+' | '+f"{x['rows']:,} | {x['INLINE_total_ms']:.1f} | {x['NATIVE_total_ms']:.1f} | {x['FGAC_total_ms']:.1f} | {percent(x['FGAC_over_NATIVE_mean'])} [{percent(x['FGAC_over_NATIVE_ci95'][0])}, {percent(x['FGAC_over_NATIVE_ci95'][1])}] | {percent(x['FGAC_over_INLINE_mean'])} |" for i,x in enumerate(s))
agg='\n'.join(f"| {label} | {percent(o[k]['paired_mean_degradation'])} | [{percent(o[k]['moving_block_ci95'][0])}, {percent(o[k]['moving_block_ci95'][1])}] | {percent(o[k]['ratio_of_total_times_minus1'])} |" for k,label in [('bob_six','Bob 六档返回比例'),('legacy_seven','原七场景等权'),('all_eight','含新增全表场景的八场景等权')])
full=s[-1];fullst={x['role']+'-'+x['mode']:x for x in st if x['case']=='whole_source_100'}
stage_table='\n'.join(f"| {k} | {v['input_records']:,.0f} | {v['input_bytes']:,.0f} | {v['executor_cpu_ms']:.1f} | {v['gc_ms']:.1f} | {v['disk_spill']:.0f} |" for k,v in fullst.items())
block='\n'.join(f"| {label} | "+' | '.join(percent(x) for x in o[k]['blocks'])+' |' for k,label in [('legacy_seven','原七场景'),('all_eight','八场景')])
text=f'''# 当前 FGAC 与 Spark + Kyuubi AuthZ + Ranger 性能复测

日期：2026-09-15。当前 FGAC 流式方案已部署到隔离实验环境，三条路径已完成统一口径复测。**资源采用实际部署形态：不限制 FGAC 两侧总资源、不绑核、不设 CPU 配额。**

## 结论

- 原七场景等权配对平均劣化为 **{percent(o['legacy_seven']['paired_mean_degradation'])}**，新增原表全量后八场景为 **{percent(o['all_eight']['paired_mean_degradation'])}**。负值表示 FGAC 更快；这是本测试集合的等权结果，不是任意业务负载的平均。
- 授权集合 100% 返回为 **{percent(s[5]['FGAC_over_NATIVE_mean'])}**，原表 100% 返回为 **{percent(full['FGAC_over_NATIVE_mean'])}**。应据此单独判断全量场景的 5% 目标，不能用混合平均替代。
- 原表全量 FGAC 相对直接填写条件的理想化参考仍有 **{percent(full['FGAC_over_INLINE_mean'])}** 的开销。该参考保留 Polaris 和同一源表读取，但没有授权插件，不是安全等价基线。
...
原表全量 FGAC Arrow 逻辑数据量中位数为 {full['FGAC_arrow_bytes']:,.0f} 字节；源扫描字节中位数为 {fullst['R-FGAC']['input_bytes']:,.0f}。因此流式传输虽避免结果 Parquet 落盘，仍需要 Parquet 解码、Arrow 输出、接收桥转换以及 E Spark 消费，流式并不意味着传输零成本。

全量 FGAC 的 E+R 进程 CPU 时间中位数为 {full['FGAC_total_executor_cpu_ms']:.1f} ms，原生 E 为 {full['NATIVE_total_executor_cpu_ms']:.1f} ms。该进程指标包含各自 Python/JVM 的工作；不是 CPU 配额，也不包含独立 Catalog/MinIO/Ranger 进程。FGAC prepare 中位数 {full['FGAC_prepare_ms']:.1f} ms、action {full['FGAC_action_ms']:.1f} ms，原生分别 {full['NATIVE_prepare_ms']:.1f} / {full['NATIVE_action_ms']:.1f} ms。阶段可能重叠，不能将 R 与 E 的墙钟耗时简单累加。
...
被动采样中 R FGAC Python+JVM 合计峰值 RSS 为 {res['R']['peak_combined_python_jvm_rss_mib']['FGAC-server']:.0f} MiB。各 E 会话峰值及主机 CPU 见 `resource-summary.json`；主机指标包含已有服务和其他活动，不能当作 FGAC 独占成本。Driver 2 GiB 是 JVM 堆设置，并非 Python、堆外内存与 JVM 合计上限。

## 复核与限制

第一轮已完成，但发现按场景预热会使原生 Alice / Bob / 全量会话只预热 5 / 30 / 5 次，而共享 FGAC、INLINE 各 40 次。该轮原七场景汇总为 {percent(initial['legacy_seven']['paired_mean_degradation'])}，只作为诊断数据保留在 `initial-*`。最终主结果来自补齐预热后的整轮重跑；没有改变 FGAC 算法、筛掉慢样本或根据结果挑选实现。即便预热次数一致，JIT 与正常运行波动仍可能存在，故同时报告区块结果。
...
'''
(p/'PERFORMANCE_REPORT.md').write_text(text,encoding='utf-8')
