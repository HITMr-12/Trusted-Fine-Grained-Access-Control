    ...
    for line in requests():
        ...
        try:
            ...
            if mode in ('FGAC', 'FGAC-LZ4', 'FGAC-ZSTD'):
                ...
                os.environ['FGAC_FLIGHT_URL'] = 'grpc://172.168.22.23:' + os.environ.get('FGAC_FLIGHT_PORT', {
                    'FGAC': '18833', 'FGAC-LZ4': '18834', 'FGAC-ZSTD': '18835'}[mode])
                bridge = FlightBridge(case['plan'], cmd.get('authorization', 'Bearer ' + case['principal'] + '-token'), prefetch=True)
                ...
                result = scan_digest(s, os.environ['MASK_DELIVERY_ROOT'], request, business,
                    host=os.environ['MASK_HOST'], port=int(os.environ.get('MASK_PORT', '19041')),
                    ca=os.environ['MASK_CA'])
