        ...
        with socket.create_connection((host, port), timeout=10) as raw:
            ...
            with conn:
                ...
                for name in names:
                    ...
                    checksum = hashlib.sha256() if sec else None
                    ...
                    if sec and checksum.hexdigest() != header.get('sha256'):
                        raise ValueError('file checksum mismatch')
                    pending.rename(out / name)
                    receipts.append({'name': name, 'size': size,
                                     'sha256': checksum.hexdigest() if sec else None})
