...
class GovernedFlightServer(flight.FlightServerBase):
    def __init__(self, location, backend, *, ticket_ttl=300, max_tickets=64, audit=None,
                 ...
                 tls_certificates=None, sec_verify=False, chunk_rows=900031):
        ...
        def produce():
            ...
            def next_batch():
                with cond:
                    ...
                    return batch
            try:
                sec_h = hashlib.sha256() if self.sec_verify else None
                sec_rows = 0      # rows accumulated in current chunk
                sec_idx = 0       # current chunk index
                pending = next_batch()
                while pending is not None:
                    ...
                    nbytes += batch.nbytes
                    if self.sec_verify:
                        # SEC chunk verification: per 900031-row chunk (1/32 of the
                        # full relation) sha256 over canonical batch bytes. Marker
                        # rides on the chunk-final batch as app_metadata.
                        sec_h.update(batch.serialize().to_pybytes())
                        sec_rows += batch.num_rows
                        boundary = rows >= (sec_idx + 1) * self.chunk_rows
                        last = pending is None
                        if boundary or last:
                            meta = json.dumps({'chunk': sec_idx, 'rows': sec_rows,
                                               'cum_rows': rows, 'last': last,
                                               'sha256': sec_h.hexdigest()}).encode()
                            sec_idx += 1
...
def main():
    ...
    parser.add_argument("--sec", choices=["on", "off"], default="off")
    parser.add_argument("--cert")
    parser.add_argument("--key")
    parser.add_argument("--chunk-rows", type=int, default=900031)
    args = parser.parse_args()
    if args.batch_rows < 1 or args.batch_rows > 65536:
        parser.error("batch-rows must be between 1 and 65536")
    if args.sec == "on" and (not args.cert or not args.key):
    ...
    if args.env_from_pid:
        ...
        # Never attach to the donor's py4j gateway: this process needs its own JVM.
        for stolen in ("PYSPARK_GATEWAY_PORT", "PYSPARK_GATEWAY_SECRET"):
            os.environ.pop(stolen, None)
    sys.path.insert(0, args.service_root)
    import remote_service as service
    backend = SparkBackend(service, args.batch_rows)
    tls = None
    if args.sec == "on":
        tls = [(Path(args.cert).read_bytes(), Path(args.key).read_bytes())]
    server = GovernedFlightServer(args.location, backend, audit=args.audit,
                                  tls_certificates=tls, sec_verify=(args.sec == "on"),
